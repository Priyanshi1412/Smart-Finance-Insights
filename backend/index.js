...
async function startServer() {
  try {
    ...
    app.get('/api/dashboard/overview', auth, async (req, res) => {
      try {
        const userId = new (require('mongoose').Types.ObjectId)(req.userId);
        const now = new Date();
        ...
        const incomeChange = prevMonthIncome > 0 ? Math.round(((curMonthIncome - prevMonthIncome) / prevMonthIncome) * 100) : curMonthIncome > 0 ? 100 : 0;
        const expenseChange = prevMonthExpenses > 0 ? Math.round(((curMonthExpenses - prevMonthExpenses) / prevMonthExpenses) * 100) : curMonthExpenses > 0 ? 100 : 0;
        ...
        const sixMonthData = sixMonthKeys.map(k => {
          const [y, mo] = k.split('-');
          return { month: k, label: new Date(y, parseInt(mo) - 1).toLocaleDateString('en-US', { month: 'short' }), income: incomeByMonth[k], expenses: expenseByMonthMap[k] };
        });
        ...
        const financialHealthScore = totalIncome > 0 ? Math.max(0, Math.min(100, Math.round((savings / totalIncome) * 100))) : 0;
        ...
        res.json({
          summary: { totalIncome, totalExpenses, savings, savingsRate, financialHealthScore, financialHealthStatus, incomeChange, expenseChange, budget: { status: budgetStatus, percentUsed: budgetPercentUsed, limit: totalBudget } },
          sixMonthData,
          categoryBreakdown: expenseByCategory.map(c => ({ category: c._id, amount: c.total })),
          transactions,
        });
      } catch (err) {
